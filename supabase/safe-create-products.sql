-- Безпечне створення таблиці продуктів
-- @file: safe-create-products.sql
-- @description: Безпечний скрипт для створення таблиці продуктів
-- @created: 2024-12-19
...
DO $$ 
BEGIN
    -- Перевіряємо та додаємо обмеження для SKU
    IF NOT EXISTS (
    ...
    -- Перевіряємо та додаємо обмеження для штрих-коду
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
DO $$
BEGIN
    RAISE NOTICE 'Таблиця products успішно створена з усіма необхідними елементами';
    RAISE NOTICE 'Індекси, тригери, функції та RLS політики налаштовані';
END $$;
